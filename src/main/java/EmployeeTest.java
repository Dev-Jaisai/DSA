...
//        });
        ...
        //5. Count the number of employees in each department.OR Get the employees name from each department having max salary

        Map<String, Optional<Employee>> collect = employeeList.stream().collect(Collectors.groupingBy(Employee::getDepartment, Collectors.maxBy(Comparator
                .comparingDouble(Employee::getSalary))));


//        collect.forEach((x,y)->{
//            System.out.println(x);
//            System.out.println(y.get().getName()+" "+y.get().getSalary());
//        });
     ...
     //11.Get the employees name start from name j
