...
public class ComparatorSort {
    public static void main(String[] args) {
        ...
        List<String> sortedNamesAndAges = el.stream()
                .sorted(Comparator.comparing(Employee::getName)
                        .thenComparing(Employee::getAge)) // Sort by name, then by age
                .map(employee -> employee.getName() + " " + employee.getAge() + "")
                .collect(Collectors.toList());

//        sortedNamesAndAges.forEach(System.out::println);

        Employee employee = el.stream().max(Comparator.comparing(Employee::getSalary)).get();

        //System.out.println(employee.getName()+" "+employee.getSalary());


        Map<String, DoubleSummaryStatistics> collect = el.stream().collect(Collectors.groupingBy(Employee::getDepartment, Collectors.summarizingDouble(Employee::getSalary)));

        //collect.forEach((x,y)-> System.out.println(x+" "+y.getMin()));

        //Employees Details Working in each department

        Map<String, List<Employee>> collect1 = el.stream()
                .collect(Collectors.groupingBy(Employee::getDepartment));

collect1.forEach((department,employees)->{
    System.out.println(department);

    employees.forEach(x-> System.out.println(x.getName()+" "+x.getSalary()));
});
    }
}
